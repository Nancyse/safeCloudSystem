			...
			<div class="agile-grids">		
				...
				<div class="asked">
					<div class="w3l-table-info">
						...
						<div style="height:60px;border-bottom:2px solid #EBEBEB;">
						
								<a href="createNewUser" class="btn btn-info btn-sm " >添加用户</a>
								<br>
								搜索：
								  用户名
								  &nbsp;<input type="text" placeholder="请输入用户名"  id="userKeyword" class="input" style="width:120px; line-height:17px;display:inline-block" /> 
								  &nbsp;&nbsp;
								  用户类型
								  <select id="userType_id" class="input" onchange="changesearch()"  style="width:60px; line-height:17px;display:inline-block">
									<option value="">选择</option>
									<option value="1">用户</option>
									<option value="2">管理员</option>									
								  </select>
								email
								  &nbsp;<input type="text" placeholder="请输入查找的email"  id="emailKeyword" class="input" style="width:120px; line-height:17px;display:inline-block" /> 
								  &nbsp;&nbsp;
								专属空间
								  &nbsp;<input type="text" placeholder="请输入查找的空间"  id="spaceKeyword" class="input" style="width:120px; line-height:17px;display:inline-block" /> 
								  &nbsp;&nbsp;								  
						...
						<tbody>
						  	...
						  	<c:forEach var="user" items="${userList}">
						  		...
						  		<td><c:out value="${user.user_space }"/></td>						  		
						  		<td> <a class="btn btn-sm btn-success" href='javascript:void(0)' onclick="downloadFile('${file.file_name }','${file.file_dir}')"><span class="icon-edit"></span>修改</a> 
						  	...
						  	<tr>
						  		...
						  		<td><c:out value="普通用户"/></td>
						  		<td><c:out value="23738379988@qq.com"/></td>
						  		<td><c:out value="lpsSpace/"/></td>						  		
						  		<td> 
						  		<a class="btn btn-sm btn-success" href='javascript:void(0)' onclick="downloadFile('${file.file_name }','${file.file_dir}')"><span class="icon-edit"></span> 修改</a>							  		
								</td>		 
							</tr>						  	
						  ...
						  <tr align="center">
							<td colspan="8" align="center">
								...
								<c:if test="${currentPage != 1 }">
									 <a href="sys-usermanage?page=${currentPage-1 }"><< 前一页</span>    
								</c:if>
								...
								<c:forEach var="i" begin="${currentPage }" end="${pageTimes }">
							        ...
							        <c:if test="${i != currentPage}">
							             <a href="sys-usermanage?page=${i}">${i}</a>
							        </c:if>
							        
								</c:forEach>								
						        <c:if test="${currentPage < pageTimes}">
						        	<a href="sys-usermanage?page=${currentPage+1}">后一页 >></a>
						        </c:if> 
						        <c:if test="${currentPage == pageTimes}">
